...
import {
  ...
  useInviteEmailToWorkspace,
  useInviteUserToWorkspace,
  useUpdateWorkspaceMember,
} from "@/src/hooks/useWorkSpaceMutation";
...
export const WorkspaceMember = () => {
  ...
  const { mutate: deleteWorkspaceMemberMutation, isPending: isPendingDeleteWorkspaceMember } =
    useDeleteWorkspaceMember();
  const { mutate: updateWorkspaceMember, isPending: isUpdateWorkspace } = useUpdateWorkspaceMember();
  const { mutate: deleteWorkspaceInvitationMutation, isPending: isPendingDeleteWorkspaceInvitation } =
    useDeleteWorkspaceInvitation();
  const { data: workspaceRoles, isLoading: isGetWorkspaceRoles } = useQuery({
    queryKey: ["workspaceRoles"],
    queryFn: async () => {
      const res = await getWorkspaceRoles();
      return res.data || [];
    },
  });
  ...
  const inviteUser = async (user: UserSearchResponse, role: string) => {
  ...
  const handleUpdateWorkspaceMember = async (memberId: number, code: string) => {
    const workspaceMemberRequest: WorkspaceMemberRequest = {
      userId: memberId,
      workspaceId: data.workspaceId,
      role: code,
    };
    console.log("update wpMem:", workspaceMemberRequest);
    updateWorkspaceMember(workspaceMemberRequest);
  };
        ...
        <ModalWrapper onClose={onClose}>
          <WorkspaceMemberModalUI
            ...
            isLoading={
              ...
              isPendingDeleteWorkspaceMember ||
              isPendingDeleteWorkspaceInvitation
            }
            ...
            handleUpdateWorkspaceMember={handleUpdateWorkspaceMember}
          />
        </ModalWrapper>
      )}
    </>
